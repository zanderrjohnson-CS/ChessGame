...
def minimax(board, depth, maximizing_player, memo):
    """
    Minimax with memoization.
    memo: dictionary storing {(board_hash, depth, maximizing): score}
    """
    # Create cache key
    ...
    if maximizing_player:  # White's turn
        ...
        for move in get_all_legal_moves(board, 'w'):
            ...
            eval = minimax(new_board, depth - 1, False, memo)
        ...
        for move in get_all_legal_moves(board, 'b'):
            new_board = make_move(board, move)
            eval = minimax(new_board, depth - 1, True, memo)
...
def ai_move_with_minimax(board, history, depth=3):
    ...
    # Create memoization dictionary for this search
    
    memo = {}
    for move in moves:
        new_board = make_move(board, move)
        score = minimax(new_board, depth - 1, True, memo)
    ...
    print(f"Positions evaluated: {len(memo)} (with caching)")
